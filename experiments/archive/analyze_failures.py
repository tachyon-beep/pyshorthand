...
for q in questions:
    ...
    if py_result and not py_result.get('is_correct'):
        ...
        # Categorize failure
        if 'implementation' in q.category or 'what happens' in q.question.lower() or 'optimization' in q.question.lower():
            failure_categories['implementation'].append(q.id)
            print('   💀 CAUSE: Requires implementation details (not in PyShorthand)')
        elif 'parameters' in q.question.lower() or 'return' in q.question.lower() or 'types are they' in q.question.lower():
