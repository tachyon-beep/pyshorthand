...
with open('/home/user/animated-system/experiments/results/multimodel_anthropic_claude-sonnet-4.5_20251123_044022.json') as f:
    results = json.load(f)
...
print('\n📊 ACCURACY:')
print(f'   Original:         {orig_metrics["correct"]}/{orig_metrics["total"]} correct ({orig_metrics["accuracy"]:.0f}%)')
print(f'   PyShorthand v1.5: {pyshort_metrics["correct"]}/{pyshort_metrics["total"]} correct ({pyshort_metrics["accuracy"]:.0f}%)')
print(f'   Difference:       {orig_metrics["accuracy"] - pyshort_metrics["accuracy"]:.1f}% (IDENTICAL!)')
...
token_reduction = (1 - pyshort_metrics['avg_prompt_tokens']/orig_metrics['avg_prompt_tokens']) * 100
...
output_cost_orig = ((orig_metrics['avg_total_tokens'] - orig_metrics['avg_prompt_tokens']) / 1_000_000) * 15
total_cost_orig = input_cost_orig + output_cost_orig

input_cost_py = (pyshort_metrics['avg_prompt_tokens'] / 1_000_000) * 3
output_cost_py = ((pyshort_metrics['avg_total_tokens'] - pyshort_metrics['avg_prompt_tokens']) / 1_000_000) * 15
...
print('SUMMARY')
print('=' * 80)
print(f'PyShorthand v1.5 delivers:')
